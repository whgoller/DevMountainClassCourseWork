...
app.controller('mainCtrl', function($scope, itunesService){
  ...
  //Also note that that method should be retuning a promise, so you could use .then in this function.
    ...
    $scope.getSongData = function (){
      itunesService.findArtistSongs($scope.artist).then(function(songs){
        console.log(songs);
        $scope.songData = massageDataFromItunes(songs);
        
        // or you can use the map function like below.
...
//            TrackName: itunesFormattedResult.trackName,
...
//        console.log(songs);
  ...
  */
  ...
  function massageDataFromItunes(data){
    ...
    //return an array of data formatted for ng-grid.
    return newArray;
  }
  ...
  //Once you do this ($scope.songData = myFinalArray) then ng-grid will see that and populate the page.
    //this is done in the above function attached to $scope.getSongData
});
